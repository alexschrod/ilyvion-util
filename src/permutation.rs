...
fn _heap_permutation<T: Copy + ToPrimitive, R: Mul<R, Output=R> + Add<R, Output=R> + Zero + NumCast>(digits: &mut [T], remaining_operations: usize, digit_length: usize, result: &mut Vec<R>) {
    if remaining_operations == 1 {
        result.push(digits.iter().fold(R::zero(), |sum, d| sum * R::from(10).unwrap() + R::from(*d).unwrap()));
        return;
    }
...
pub fn heap_permutation<T: Copy + ToPrimitive, R: Mul<R, Output=R> + Add<R, Output=R> + Zero + NumCast>(digits: &mut [T]) -> Vec<R> {
